...
@section('content')
    <div class="content">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h1 class="mb-0"><i class="fas fa-tasks"></i> Tareas y Asignaciones</h1>
            <form id="filterForm" action="{{ route('tasks.index') }}" method="GET" class="d-inline-block">
        ...
        <div class="d-flex flex-wrap mb-3">
            <form action="{{ route('tasks.index') }}" method="GET" class="mr-2 flex-grow-1 d-flex">
                <input type="text" name="search" placeholder="Buscar tarea..." class="form-control"
                    style="width: 60%;">
                <button type="submit" class="btn btn-primary ml-2 mt-md-0">Buscar</button>
                <a href="{{ route('tasks.index') }}" class="btn btn-secondary ml-2 mt-md-0">Borrar Filtros</a>
            </form>
        ...
        <div id="addTaskModal" class="modal" style="display: {{ $errors->any() ? 'block' : 'none' }};">
        ...
        <div class="container">
            <h1>Lista de Tareas y Asignaciones</h1>
            @if (session('success'))
                    ...
                    <tbody>
                        @forelse($tasks as $task)
                            <tr>
                                ...
                                <td>
                                    @if($task->file_path)
                                    <a href="{{ asset('storage/' . $task->file_path) }}" target="_blank">Ver Archivo</a>
                                    @else
                                        No hay archivo
                                    @endif
                                </td>
                                <td>{{ $task->percentage }}%</td> <!-- Mostrar el porcentaje -->
                                <td>
                                    <button class="btn btn-sm" style="background-color: transparent;"
                                        onclick="openViewTaskModal({{ json_encode($task) }})">
                                    ...
                                    <a href="{{ route('add-grades-tasks', ['courseId' => $task->course_id, 'cycle' => $task->cycle]) }}" class="btn btn-sm btn-primary">
...
<script>
    ...
    function openViewTaskModal(task) {
        ...
        document.getElementById('viewCourse').innerText = task.course.name; // Mostrar el nombre del curso
        document.getElementById('viewCycle').innerText = task.cycle; // Mostrar ciclo
        document.getElementById('viewInstitution').innerText = task.course.institution; // Mostrar institución
    ...
    function openEditTaskModal(task) {
        ...
        document.getElementById('edit-percentage').value = task.percentage;

        // Precargar el curso y ciclo
        document.getElementById('edit-course').value = task.course_id;
        updateEditCycleOptions(task.course_id, task.cycle);
    ...
    // Actualizar el porcentaje permitido para tareas
    ...
    function fetchTasks(studentId) {
        fetch(`/students/${studentId}/tasks`)
            .then(response => response.json())
            .then(data => {
                ...
                data.tasks.forEach(task => {
                    ...
                    div.innerHTML = `
                        <label for="task_${task.id}">${task.name} (${task.percentage}%)</label>
                        <input type="number" class="form-control" id="task_${task.id}" name="grades[${task.id}]" min="0" max="100" required>
                    `;
